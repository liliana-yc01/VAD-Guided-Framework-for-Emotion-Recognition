...
CHECKPOINT_FILE = "ekman_emotion_llm_full_results.csv"
...
target_emotions = ["angry", "afraid", "disgusted", "joyful", "sad", "surprised"]
...
vad_coords = {
    ...
    "disgusted": np.array([-0.60, 0.35, 0.11]),
    "joyful": np.array([0.76, 0.48, 0.35]),
    "sad": np.array([-0.63, -0.27, -0.33]),
    "surprised": np.array([0.00, 0.67, -0.13]),
}
...
# ---------------------------------------------------------------------
# 4. Sample exactly 1940 instances per target emotion (1940 x 6 = 11640 samples)
# ---------------------------------------------------------------------
...
- disgusted: [V: -0.60, A: +0.35, D: +0.11] -> High displeasure, moderate arousal, slight avoidance/control (rejection).
...
- surprised: [V: 0.00, A: +0.67, D: -0.13] -> Neutral or mixed (can be either positive or negative) valence, high arousal, low/uncertain control (startle).

**CRITICAL RULE 1: PRIMACY OF EXPLICIT EMOTION WORDS**
If the speaker uses an explicit emotion word (e.g., “mad”, “scared”, “afraid”, “happy”, “sad”, “joyful”, “surprised”, “disgusted”, “shocked”, “amazed”, “terrified”), that label MUST be your final prediction. Use VAD only to explain, not to override.
...
**CRITICAL RULE 3: SURPRISE DISTINCTION**
   - Surprise is about **unexpectedness** and **high arousal** – valence can be positive or negative.
   - If the situation is clearly positive and the speaker says "happy" or "excited" → choose **joyful**.
   - If the situation is positive but the speaker says "shocked", "astonished", or "couldn't believe it" and does **not** say "happy" or "excited" → choose **surprised**.
   - If the situation is negative but the speaker says "shocked" or "amazed" without a happiness/grief word → choose **surprised**.
   
**CRITICAL RULE 4: ANGER vs. FEAR (The Blame Test)**
...
**CRITICAL RULE 5: SADNESS vs. ANGER (The Energy Test)**
...
**CRITICAL RULE 6: DISGUSTED vs. ANGER – REVULSION TEST**
- If the event involves **contamination, bad taste, bodily fluids, or visual repulsion**, choose **disgusted** – even if there is also blame.
- *Example:* “The restaurant served me raw chicken – I was disgusted” → **disgusted** (revulsion is primary).
- *Example:* “My roommate ate my leftovers without asking” → **anger** (no revulsion, pure irritation).

**CRITICAL RULE 7: THRILL & ANTICIPATION CONTEXTS**
...
Situation: "I bit into an apple and found a worm wiggling in the core."
REASONING: Unexpected contamination causes strong disgust, negative valence, moderate arousal, and a desire to reject/avoid → disgusted.
VAD_Estimate: [V: -0.60, A: 0.45, D: 0.10]
EMOTION: disgusted
...
Situation: "I was shocked when my parents gave me a car for graduation."
REASONING: Explicit “shocked” indicates surprise; the event is positive but the dominant reaction is disbelief, not sustained joy.
VAD_Estimate: [V: +0.40, A: +0.70, D: -0.10]
EMOTION: surprised

Situation: "I opened the refrigerator to grab my lunch, but the leftovers I was saving were completely gone."
REASONING: The missing food is unexpected and momentarily startling. There is no clear threat or blame yet, so it is not anger. The valence is mildly negative but the dominant reaction is confusion and brief high arousal → surprised.
VAD_Estimate: [V: -0.20, A: +0.60, D: -0.30]
EMOTION: surprised
...
# 14. Save Outputs
# ---------------------------------------------------------------------
df_sample.to_csv("ekman_emotion_llm_full_results.csv", index=False)

baseline_confusion_df.to_csv("ekman_baseline_confusion_matrix.csv")
theory_confusion_df.to_csv("ekman_theory_guided_confusion_matrix.csv")
...
baseline_errors.to_csv("ekman_baseline_error_analysis.csv", index=False)
theory_errors.to_csv("ekman_theory_guided_error_analysis.csv", index=False)

helped_cases.to_csv("ekman_theory_helped_cases.csv", index=False)
hurt_cases.to_csv("ekman_theory_hurt_cases.csv", index=False)
both_wrong_cases.to_csv("ekman_both_wrong_cases.csv", index=False)

print("\nFiles saved:")
print("- ekman_emotion_llm_full_results.csv")
print("- ekman_baseline_confusion_matrix.csv")
print("- ekman_theory_guided_confusion_matrix.csv")
print("- ekman_baseline_error_analysis.csv")
print("- ekman_theory_guided_error_analysis.csv")
print("- ekman_theory_helped_cases.csv")
print("- ekman_theory_hurt_cases.csv")
print("- ekman_both_wrong_cases.csv")
